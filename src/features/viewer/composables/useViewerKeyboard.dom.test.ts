...
describe('useViewerKeyboard', () => {
  ...
  it('handles G key with shift to go to last page', () => {
    const { state } = setup();

    press('G', { shiftKey: true });

    expect(state.currentIndex.value).toBe(state.totalPages.value - 1);
    expect(state.showFeedback).toHaveBeenCalled();
  });

  it('handles g key without shift to go to first page', () => {
    const { state } = setup();
  ...
  it('toggles auto-scroll with a or A key', () => {
    const { state } = setup();
    const autoScroll = { toggle: vi.fn(), pause: vi.fn() };
    const wrapper = mountLifecycle(() => useViewerKeyboard(state, autoScroll as never));
    wrappers.add(wrapper);
  ...
  it('ignores a/A with modifier keys', () => {
    const { state } = setup();
    const autoScroll = { toggle: vi.fn(), pause: vi.fn() };
    const wrapper = mountLifecycle(() => useViewerKeyboard(state, autoScroll as never));
    wrappers.add(wrapper);
    ...
    expect(autoScroll.toggle).not.toHaveBeenCalled();
  });

  it('pauses auto-scroll on manual navigation keys', () => {
    const { state } = setup();
    const autoScroll = { pause: vi.fn(), toggle: vi.fn() };
    const wrapper = mountLifecycle(() => useViewerKeyboard(state, autoScroll as never));
    wrappers.add(wrapper);

    press('PageDown');

    expect(autoScroll.pause).toHaveBeenCalledWith('interaction');
  });
});
